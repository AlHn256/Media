...
namespace AlfaPribor.VideoCameras4
{
    ...
    /// <summary>Класс "камера Basler"</summary>
    public class BaslerTelecamera : VideoCameraBase
    {
        ...
        /// <summary>Конструктор класса телекамеры Basler</summary>
        /// <param name="settings">Параметры IP камеры</param>
        public BaslerTelecamera(IpCameraSettings settings) : base(settings)
        {
        ...
        bool Connect()
        {
            ...
            try
            {
                basler = new BaslerVideo(FHost, FUsername, FPassword, 
                                         settings.Resolution, settings.FPS.ToString(), settings.Compression.ToString());
                basler.Channel = FChannel;
                basler.OnFrame += new EventHandler<BaslerVideoFrameEventArgs>(basler_OnFrame);
                basler.OnStatus += new EventHandler<BaslerVideoStatusEventArgs>(basler_OnStatus); 
        ...
        void basler_OnStatus(object sender, BaslerVideoStatusEventArgs e)
        {
            if (e.Status == BaslerVideoStatus.Online)
            {
                State = VideoCameraStatus.Online;
            }
            else if (e.Status == BaslerVideoStatus.Offline ||
                e.Status == BaslerVideoStatus.AccessDenied ||
                e.Status == BaslerVideoStatus.BadRequest ||
                e.Status == BaslerVideoStatus.Error)
            {
                State = VideoCameraStatus.Offline;
            }
            else if (e.Status == BaslerVideoStatus.Inactive)
            {
                State = VideoCameraStatus.Unknown;
            }
        }

        void basler_OnFrame(object sender, BaslerVideoFrameEventArgs e)
        {
        ...
        void Disconnect()
        {
            ...
            basler.Active = false;
            basler.OnFrame -= basler_OnFrame;
            basler.OnStatus -= basler_OnStatus;
            basler.Dispose();
            basler = null;
        }
