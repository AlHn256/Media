...
namespace AlfaPribor.VideoStorage2
{
    ...
    public class AskoVideoRecord : BaseVideoRecord
    {
        ...
        /// <param name="status">Статус запрашиваемого интерфейса</param>
        public AskoVideoRecord(string id, int partition_id, VideoStorageIntStat status)
                ...
                switch (mode)
                {
                    case VideoRecordOpenMode.Read:
                        FramesStream = new BufferedStream(
                            new FileStream(path + id + ".mjpg", FileMode.Open, FileAccess.Read, FileShare.Read),
                            65536
                        );
                        break;
                    ...
                    case VideoRecordOpenMode.ReadWrite:
                        FramesStream = new BufferedStream(
                            new FileStream(path + id + ".mjpg", FileMode.OpenOrCreate, FileAccess.ReadWrite, FileShare.ReadWrite),
                            65536
                        );
                        break;
                    ...
                    while (indexes.ReadIndex(out index) != 0)
                    {
                        ...
                        if (i < 0)
                        {
                            ...
                            if (firstFrameIndexes.Count > 0)
                            {
                                ...
                                {
                                    already_exist = firstFrameIndexes[i - 1].StreamId == index.StreamId ||
                                        firstFrameIndexes[i].StreamId == index.StreamId;
                                }
                            }
                            if (!already_exist)
                            {
