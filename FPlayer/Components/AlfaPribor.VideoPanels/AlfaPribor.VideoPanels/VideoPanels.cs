...
namespace AlfaPribor.VideoPanels
{
    public class VideoPanels
    {
        ...
        public enum VideoStyle
        {
            ...
            Hexagon = 6,
            /// <summary>Три видеоокна одна строка</summary>
            TripleOneRow = 7,
        ...
        /// <summary>Панели отображения видео</summary>
        Panel[] PanelVideo;
        /// <summary>Видео разщмещать на панелях</summary>
        ...
        void PushProportions(float proportions, int windowsCount)
        {
            ...
            for (int i = 0; i < windowsCount; i++)
                VideoAspectRatio.Add(proportions);
        }
        ...
        private PositionArray _tablePositionArray;
        ...
        public PositionArray TablePositionArray
        {
            get
            {
                if (_tablePositionArray == null)
                {
                    _tablePositionArray = new PositionArray();
                }
                return _tablePositionArray;
            }
        }

        private PositionArray _rightTablePositionArray;
        ...
        public PositionArray RightTablePositionArray
        {
            get
            {
                if (_rightTablePositionArray == null)
                {
                    _rightTablePositionArray = new PositionArray();
                }
        ...
        private List<bool> RotationStates
        {
            get
            {
        ...
        /// <summary>Единица измерения предельных положений сплиттера </summary>
        ...
        public VideoPanels(VideoStyle style, Control Owner, double SplitterSize,
        ...
        public VideoPanels(VideoStyle style, Control Owner,
                          double SplitterSize, bool ExternalWindow,
                          int MinSplitterPercent, int MaxSplitterPercent,
                          float VideoProportions, bool ShowCaptions, bool EnablePanels, MestureUnit unit = MestureUnit.Persantage)
            : this()
        {
            CanCheckPanelHeight = true;
            _rotationStates = new List<bool>();
        ...
        public VideoPanels(VideoStyle style, Control Owner,
                          double SplitterSize, bool ExternalWindow,
                          int MinSplitterPercent, int MaxSplitterPercent,
                          float VideoProportions, bool ShowCaptions, bool EnablePanels, List<bool> rotationStates, MestureUnit unit = MestureUnit.Persantage)
        {
            CanCheckPanelHeight = true;
        ...
        public VideoPanels(VideoStyle style, Control Owner, double SplitterSize, bool HiddenCaptions,
        ...
        private void InitMinMaxBarier(float MinSplitterPercent, float MaxSplitterPercent)
        {
                                 ...
                                 bool ExternalWindow, bool showCaptions, bool enablePanels)
        {
            ...
            CreateTimers(); //Создание вспомогательных таймеров

            #region Создать панели видеоокон
            ...
            for (int i = 0; i < windows_count; i++)
            {
                ...
                if (style == VideoStyle.TripleOneRow || !RotationStates[i])
                {
                    // VideoContainer[i].Anchor = AnchorStyles.Left | AnchorStyles.Right | AnchorStyles.Top;
                    VideoContainer[i].Dock = DockStyle.Fill;
                }
                else
                {
                    // VideoContainer[i].Anchor = AnchorStyles.Left |AnchorStyles.Top|AnchorStyles.Bottom;
                    VideoContainer[i].Dock = DockStyle.Fill;//Необходимо для использования поиска по контролу иначе не ищет
                }
                ...
                if (style == VideoStyle.TripleOneRow)
                {
            ...
            if (style == VideoStyle.TripleOneRow) SetRotationTripleWindows();   //Три окна с поворотом
        ...
        public void SetVideoProportions()
        {
            ...
            try
            {
                ...
                if (Mode == ScreenMode.PolyScreen)
                {
                    ...
                    int columns = CalcColumnCount(2);   //Число колонок видеоокон
                    ...
                    //Есть количество стобцов, зачем ещё условия. Стиль парели учтён в методе получения количества колонок
                    for (int i = 0; i < columns; i++)
                        tspFirst.ColumnStyles[i].Width = tspFirst.DisplayRectangle.Width / columns;

                    //Первое видеоокно (Панель кнопок управления видео)

                    #region Окна с заголовками (старый вариант)
                    if (show_captions)
                    {
                        //Кнопки верхних окон
                        tspFirst.RowStyles[0].Height = ToolBarHeight;
                        //Первый ряд видеоокон
                        float row_height = tspFirst.ColumnStyles[0].Width * GetVideoAspectRatio(0);

                        //Повернутые телекамеры
                        if (videostyle == VideoStyle.TripleOneRow || videostyle == VideoStyle.DoubleRotation || CanRotateDoubleWinows(0))
                        {
                            row_height = tspFirst.ColumnStyles[0].Width * (1.0f / GetVideoAspectRatio(0));
                        }
                        if (IsRotationDoubleMode())
                        {
                            row_height = tspFirst.ColumnStyles[0].Width * (1.0f / GetVideoAspectRatio(0));
                        }
                        tspFirst.RowStyles[1].Height = row_height;

                        //Если число окон более или равно трем
                        if (VideoContainer.Length >= 3)
                        {
                            //Кнопки нижних окон
                            tspFirst.RowStyles[2].Height = ToolBarHeight;
                            //Второй ряд видеоокон
                            tspFirst.RowStyles[3].Height = tspFirst.ColumnStyles[0].Width * GetVideoAspectRatio(3);

                            //Повернутые телекамеры
                            if (videostyle == VideoStyle.TripleOneRow)
                            {
                                // tspFirst.RowStyles[3].Height = tspFirst.ColumnStyles[0].Width * (1.0f / VideoAspectRatio);
                                tspFirst.RowStyles[3].Height = tspFirst.ColumnStyles[0].Width * (1.0f / GetVideoAspectRatio(3));
                            }
                            tspFirst.RowStyles[4].Height = splitCtrlMain.ClientSize.Height -
                                                         (2 * ToolBarHeight) -
                                                         (2 * tspFirst.ColumnStyles[0].Width * GetVideoAspectRatio(0));
                        }
                    }
                    #endregion

                    #region Окна без заголовков (обычный вариант)
                    else
                    {
                        //Первый ряд видеоокон
                        tspFirst.RowStyles[0].Height = tspFirst.ColumnStyles[0].Width * GetVideoAspectRatio(0);

                        //Повернутые телекамеры
                        if (videostyle == VideoStyle.TripleOneRow || videostyle == VideoStyle.DoubleRotation || CanRotateDoubleWinows(0))
                        {
                            if (RotationStates.Contains(true))
                                tspFirst.RowStyles[0].Height = tspFirst.ColumnStyles[0].Width * (1.0f / GetVideoAspectRatio(0));
                            else
                                tspFirst.RowStyles[0].Height = tspFirst.ColumnStyles[0].Width * GetVideoAspectRatio(0);
                        }

                        //Если число окон более трех
                        if (VideoContainer.Length >= 3 || IsRotationDoubleMode())
                        {
                            //Второй ряд видеоокон
                            tspFirst.RowStyles[1].Height = tspFirst.ColumnStyles[0].Width * GetVideoAspectRatio(1);
                            //Повернутые телекамеры
                            if (videostyle == VideoStyle.TripleOneRow)
                            {
                                tspFirst.RowStyles[1].Height = tspFirst.ColumnStyles[0].Width * (1.0f / GetVideoAspectRatio(0));
                            }
                            //Ряд панели плеера
                            if (tspFirst.RowStyles.Count > 2)
                                tspFirst.RowStyles[2].Height = splitCtrlMain.ClientSize.Height -
                                                               (2 * tspFirst.ColumnStyles[0].Width * GetVideoAspectRatio(0));
                        }
                    }
                    #endregion
                    ...
                    if (tspSecond != null)
                    {
                        ...
                        if (show_captions)
                        {
                            ...
                            //Высота видеоокна
                            //float row_height = tspSecond.ColumnStyles[0].Width * VideoAspectRatio;
                            float row_height = tspSecond.ColumnStyles[0].Width * GetVideoAspectRatio(0);
                            //Повернутые телекамеры
                            if (videostyle == VideoStyle.TripleOneRow || CanRotateDoubleWinows(1))
                            {
                                //row_height = tspSecond.ColumnStyles[0].Width * (1.0f / VideoAspectRatio);
                                row_height = tspSecond.ColumnStyles[0].Width * (1.0f / GetVideoAspectRatio(0));
                            }

                            tspSecond.RowStyles[1].Height = row_height;
                        }
                        //Окна без заголовков
                        else
                        {
                            // tspSecond.RowStyles[0].Height = tspSecond.ColumnStyles[0].Width * VideoAspectRatio;
                    ...
                    //for (int i = 0; i < VideoContainer.Length; i++) 
                    //    if (EventPanelResize != null) EventPanelResize(i);

                }
                if (Mode == ScreenMode.OneScreen)
                {
                    MaximizeWindow(MaxWindowIndex);
                }
                SetReversiveView();
            }
            catch { };
        ...
        int CalcColumnCount(int cnt)
        {
            ...
            if (videostyle == VideoStyle.Hexagon || videostyle == VideoStyle.TripleOneRow) return 3;
        ...
        public void SetSplitterSecondPos(double pos)
        {
            try 
            { 
                if (splitRight != null) splitRight.SplitterDistance = (int)(splitRight.Height * pos); }
            catch { };
        }
        ...
        /// <summary>Возвращает массив контейнеров размещения видео</summary>
        ...
        public void SetStaticImage(int window, Bitmap bmp)
        {
            try
            {
                if (enable_panels)
                {
                    ...
                    //    PanelVideo[window].BackgroundImage = bmp;
                ...
                else
                {
                    ...
                    //    PictureBoxVideo[window].Image = bmp;
        ...
        void SetOneWindow()
        {
            ...
            //Положение панели плеера - внизу левой панели
        ...
        /// <summary>Создание интерфейса с 3-мя видеоокнами</summary>
        void SetTripleWindows()
        {
            ...
            if (show_captions)
            {
                //Панель плеера в правом нижнем квадранте
                tspFirst.SetRow(PanelPlay, 3);
                tspFirst.SetColumn(PanelPlay, 1);
                tspFirst.SetRowSpan(PanelPlay, 2);//Плеер располагается в двух строках
            }
            else
            {
                //Панель плеера в правом нижнем квадранте
        ...
        void SetRotationTripleWindows()
        {
            ...
            if (show_captions)
            {
                tspFirst.SetRow(PanelPlay, 2);
                tspFirst.SetColumn(PanelPlay, 0);
                tspFirst.SetColumnSpan(PanelPlay, 3);
            }
            //Без заголовков
            else
            {
        ...
        bool ShowSecondWindow()
        {
            ...
            ExternalPictureBoxVideo.BackColor = Color.AliceBlue;
        ...
        void CreateTable(int windows_count)
        {
            ...
            #region Интерфейс БЕЗ поворота - классический
        ...
        public bool ResetElement(int oldPosition, int newPosition)
        {
            ...
            if (videostyle == VideoStyle.TripleOneRow || IsRotationDoubleMode())
            {
                ...
                Size oldSize = VideoWindows[oldPosition].Size;
                Size newSize = VideoWindows[newPosition].Size;
        ...
        public void MaximizeWindow(int index)
        {
            try
            {
                ...
                */
                ...
                if (videostyle == VideoStyle.Hexagon)
                {
                    VideoWindows[index].Dock = DockStyle.None;      //Отвязка границ
                    VideoWindows[index].Anchor = AnchorStyles.None; //Отвязка границ
                    int height = h;//Высота остается
                    int width = (int)Math.Round(h * (1 / VideoAspectRatio[index]));//Ширина видеоконтейнера - по пропорции
                    VideoWindows[index].Size = new Size(width, height);
                    VideoWindows[index].Location = new Point((tspFirst.Width - width) / 2, 0);
        ...
        {
            int len = VideoWindows[i].Width;
            Size vwSize = new Size(w, h);
            tspFirst.ColumnStyles[i].Width = tspFirst.Width;

            VideoWindows[i].Anchor = AnchorStyles.None;
            //VideoWindows[i].Dock = DockStyle.None;

            VideoWindows[i].Size = vwSize;
            VideoWindows[i].Location = new Point((tspFirst.Width - w) / 2, VideoWindows[i].Location.Y);

            VideoWindows[i].Anchor = AnchorStyles.Left | AnchorStyles.Bottom | AnchorStyles.Right | AnchorStyles.Top;
            //VideoWindows[i].Dock = DockStyle.None;
