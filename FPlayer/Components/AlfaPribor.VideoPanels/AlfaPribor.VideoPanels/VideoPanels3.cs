...
namespace AlfaPribor.VideoPanels
{
    ...
    /// версия: 3, дата: ноябрь 2021
    /// Корректировка 3 оконного режима (с не повернутыми камерами)
    /// Возможность установки от 2 до 6 видеоокон
    /// Интерфейс с поворотом для 3, 4 телекамер и 6 телекамер
    /// </summary>
    public class VideoPanels3
    {
        ...
        int splitterStepSize = 20;

        /// <summary>Панель плеера для 3-оконного режима без поворота</summary>
        Panel EmptyPanel;
        ...
        public VideoPanels3(VideoStyle style, Control Owner, double SplitterSize,
        ...
        public VideoPanels3(VideoStyle style, Control Owner,
        ...
        public VideoPanels3(VideoStyle style, Control Owner, double SplitterSize, bool HiddenCaptions,
                                 ...
                                 bool ExternalWindow, bool showCaptions, bool enablePanels)
        {
            ...
            for (int i = 0; i < windows_count; i++)
            {
                ...
                if (style == VideoStyle.TripleOneRow || style == VideoStyle.HexagonRotation /* || style == VideoStyle.DoubleRotation*/)
                {
                ...
                //Отступы для двух окон с поворотом
                if (style == VideoStyle.DoubleRotation)
                {
                    if (i == 0) VideoContainer[i].Margin = new Padding(0, 0, 1, 0);
                    if (i == 1) VideoContainer[i].Margin = new Padding(1, 0, 0, 0);
                }
        ...
        void SetDoubleRotationWindows()
        {
            ...
            //splitRight.SplitterDistance = splitCtrlMain.Panel2.Height - 200;
        ...
        void SetTripleWindows()
        {
            ...
            //SetPanelPlayer(show_captions, 3, 2, 0);

            SetPanelPlayer(show_captions, 3, 1, 1);

            //Пустая панель для 3 не повернутых видеоокон
            if (EmptyPanel == null) EmptyPanel = new Panel();
            EmptyPanel.Dock = DockStyle.Fill;
            tspFirst.Controls.Add(EmptyPanel);
            tspFirst.SetRow(EmptyPanel, 2);
            tspFirst.SetColumn(EmptyPanel, 0);
            tspFirst.SetColumnSpan(EmptyPanel, 1);
        ...
        void SetTripleRotationWindows()
        {
            ...
            CreateTable(3);
            //Размещение панели плеера
            SetPanelPlayer(show_captions, 2, 1, 0, 3);
            //Панель информации на вторую половину сплиттера
            splitCtrlMain.Panel2.Controls.Add(PanelInfo);
            //Установка размеров окна
            SetVideoProportions();
        ...
        void SetSixRotationWindows()
        {
            ...
            //Размещение панели плеера
            SetPanelPlayer(show_captions, 2, 1, 0);
            //Расширить панель плеера на 3 колонки
        ...
        /// <param name="column">Колонка размещения плеера</param>
        ...
        /// <param name="without_captions">Строка плеера для интерфейса без заголовков</param>
        /// <param name="column">Колонка размещения плеера</param>
        /// <param name="rows">Число колонок на которые растягивается плеер</param>
        /// <remarks>Добавлено для режима 3-окна с поворотом, возможно распространить на остальные режимы, 
        /// так как для них SetColumnSpan устанавливается сомнительно</remarks>
        void SetPanelPlayer(bool show_captions, int pos_captions, int without_captions, int column, int rows)
        {
            //Размещение панели плеера
            if (show_captions) tspFirst.SetRow(PanelPlay, pos_captions);//С заголовками
            else tspFirst.SetRow(PanelPlay, without_captions);
            tspFirst.SetColumn(PanelPlay, column);
            tspFirst.SetColumnSpan(PanelPlay, rows);
        }
        ...
        void CreateTable(int windows_count)
        {
            ...
            if (videostyle != VideoStyle.TripleOneRow && videostyle != VideoStyle.HexagonRotation && !IsRotationDoubleMode())
            {
                ...
                if (tspFirst == null)
                {
                    ...
                    if (show_captions) for (int i = 0; i < 5; i++) tspFirst.RowStyles.Add(new RowStyle(SizeType.Absolute, 20));
                    else for (int i = 0; i < 3; i++) tspFirst.RowStyles.Add(new RowStyle(SizeType.Absolute, 20));
            ...
            if (videostyle == VideoStyle.DoubleRotation || videostyle == VideoStyle.TripleOneRow || videostyle == VideoStyle.HexagonRotation)
            {
                //Первая коллекция видеоокон
                if (tspFirst == null)
                {
        ...
        public void SetVideoProportions()
        {
            ...
            try
            {
                ...
                if (Mode == ScreenMode.PolyScreen)
                {
                    ...
                    int row_height = (int)Math.Round(column_width * GetVideoAspectRatio());
                    ...
                    if (videostyle == VideoStyle.TripleOneRow || videostyle == VideoStyle.DoubleRotation || videostyle == VideoStyle.HexagonRotation ||
                        ...
                        row_height = (int)Math.Round(column_width * (1.0f / GetVideoAspectRatio()));

                    int row = 0;
                    if (show_captions) row = 1;
                    else tspFirst.RowStyles[row].Height = row_height;

                    //Режим три повернутых окна
                    if (VideoContainer.Length >= 3 && videostyle == VideoStyle.TripleOneRow)
                    {
                        //Кнопки нижних окон
                        tspFirst.RowStyles[1].Height = tspFirst.ColumnStyles[0].Width * (1.0f / GetVideoAspectRatio());
                    }
                    ...
                    if (tspSecond != null)
                    {
                        ...
                        if (show_captions)
                        {
                            ...
                            //Высота видеоокна
                            float row_height_2 = tspSecond.ColumnStyles[0].Width * GetVideoAspectRatio();
                            //Повернутые телекамеры
                            if (videostyle == VideoStyle.TripleOneRow || CanRotateDoubleWinows(1))
                                row_height_2 = tspSecond.ColumnStyles[0].Width * (1.0f / GetVideoAspectRatio());
                        ...
                        {
                            tspSecond.RowStyles[0].Height = tspSecond.ColumnStyles[0].Width * GetVideoAspectRatio();
                            if (videostyle == VideoStyle.TripleOneRow || CanRotateDoubleWinows(1))
                                tspSecond.RowStyles[0].Height = tspSecond.ColumnStyles[0].Width * (1.0f / GetVideoAspectRatio());
                        }
                    }
                    #endregion
            ...
            {
                {
                    ...
                    else
                    {
                        ...
                        tPanel = VideoContainer[i].Parent as TableLayoutPanel;
                        //if (tPanel == null) tPanel = VideoContainer[i].Parent.Parent as TableLayoutPanel;
                        ...
                        if (tPanel != null)
                        {
                            ...
                            VideoContainer[i].Dock = DockStyle.None;//DockStyle.Fill;
        ...
        public void MaximizeWindow(int index)
        {
            try
            {
                ...
                if (videostyle == VideoStyle.Hexagon)
                {
                    ...
                    //Расчет высоты окна
                    h = (int)Math.Round((width * GetVideoAspectRatio()) + (width * GetVideoAspectRatio()));//Высота двух видеоокон
                }
                ...
                */
                ...
                //Центровка видеоокна в случае 3-ти видеоокон
                if (videostyle == VideoStyle.TripleTwoRows)
                {
                    VideoContainer[index].Dock = DockStyle.None;      //Отвязка границ
                    VideoContainer[index].Anchor = AnchorStyles.None; //Отвязка границ
                    int height = h - 90;//Высота остается
                    int width = (int)Math.Round(height * (1 / VideoAspectRatio[index]));//Ширина видеоконтейнера - по пропорции
                    //Установлена высота нужной строки
                    tspFirst.RowStyles[GetMaxRow(index)].Height = height; 

                    VideoContainer[index].Size = new Size(width, height);
                    //VideoContainer[index].Location = new Point((tspFirst.Width - width) / 2, 0);
                    VideoContainer[index].Location = new Point(0, 0);
                    //VideoContainer[index].Left = 0;
                    //VideoContainer[index].Top = 0;
                }
                ...
                int r = 1;
                //или вторую для 3 оконного режима
                if (videostyle == VideoStyle.TripleTwoRows) r = 3;
                tspFirst.SetRowSpan(PanelPlay, r);
