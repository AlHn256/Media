...
{
    public partial class FormSpeedGraph : Form
    {
        ...
        public FormSpeedGraph()
        {
            InitializeComponent();
        }

        public FormSpeedGraph(IVideoIndex index)
            : this()
        {
            _videoIndex = index;
        ...
        private void DrawData(PointPairList list)
        {
            this.Invoke((MethodInvoker)(() =>
            {
                ...
                pane.Title.Text = "Скорость вагона";
                pane.XAxis.Title.Text = "Индекс кадра (TimeStamp/10)";
                pane.YAxis.Title.Text = "Скорость вагона";
        ...
        private void GetFileIndexes()
        {
            try
            {
                //Необходимо вызвать Refresh перед запуском
                 List<WagonDataShort> wagons = new List<WagonDataShort>();
                 wagons.AddRange(SettingContainer.WagonList.WagonDataList);
                 if (wagons.Count == 0)
                 {
                     MessageBox.Show("Вагоны не обнаружены");
                     this.Close();
                 }
                ...
                {
                    int wagonSpeed=0;
                    if (wagons[currentWagonIndex].TimeSpan < i)
                        currentWagonIndex++;
                    if (currentWagonIndex >= (wagons.Count))
                        break;
                    int halfWagonTime = wagons[currentWagonIndex].TimeSpanBegin + (wagons[currentWagonIndex].TimeSpan - wagons[currentWagonIndex].TimeSpanBegin)/2;
                    wagonSpeed = (i < halfWagonTime ? wagons[currentWagonIndex].SpeedBegin : wagons[currentWagonIndex].SpeedEnd);
                    int cadrTime = index.GetFrameTime(_channelID, i);
                    graphData.Add(new PointPair(frameCounter++, wagonSpeed));
                }
                DrawData(graphData);
